...
grad_mag_zarr_path = None # f'{dataset_path}/lasagna_inputs/las_008_grad_mag.ome.zarr'
normal_zarr_group = '4'
...
surf_sdt_zarr_path = None # f'{dataset_path}/lasagna_inputs/las_008_surf_sdt.ome.zarr'
surf_sdt_zarr_group = None # '1'
pcl_json_paths = [
...
fibers_path = None # f'{dataset_path}/fibers'
verified_patches_path = f'{dataset_path}/verified_patches'
unverified_patches_path = None # f'{dataset_path}/unverified_patches'
run_tag = os.environ.get('FIT_SPIRAL_RUN_TAG')
shell_path = f'{dataset_path}/outer_shell'
tracks_dbm_path = None # f'{dataset_path}/tracks/2um_ds2_ps256_surf_v2.dbm'  # or: m7_ds2_z3000_18000_surf.dbm
...
default_config = {
    ...
    'use_unverified_patches': True,
        'use_normals': False, # True,
    'use_surf_sdt': False, # True,
    'use_tracks': True,
    'use_gradient_magnitude': False, # True,
    'use_fibers': True,
    'random_seed': 1,
...
def main(load_only_patches_and_point_collections=False, interactive_driver=None):
    ...
    def load_patches_from_dir(path, label='patches'):
        ...
        progress = progress_or_null(None)
        patches = {}
        entries = sorted(os.listdir(path))
        progress.begin(
            'loading', f'Loading {label}',
            step=0, total_steps=len(entries), unit='patches')
        for entry_number, entry in enumerate(entries[::50], start=1):
            segment_path = os.path.join(path, entry)
            try:
                patches[entry] = load_tifxyz(segment_path)
            except Exception as e:
