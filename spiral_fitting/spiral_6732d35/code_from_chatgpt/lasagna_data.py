...
def _prepare_lasagna_volume_fast(
    scroll_zarr, *, use_normals, use_spacing, normal_nx_zarr_path,
    normal_ny_zarr_path, grad_mag_zarr_path, normal_zarr_group, z_begin,
    z_end, lasagna_scale,
):
    """Legacy bounded-cache loader accelerated by ``fast_cache``."""
    group = str(normal_zarr_group)
    reference_shape = None
    if use_normals:
        nx_array = zarr.open(normal_nx_zarr_path, mode="r")[group]
        ny_array = zarr.open(normal_ny_zarr_path, mode="r")[group]
        ...
        if nx_array.dtype != np.dtype("uint8") or ny_array.dtype != np.dtype("uint8"):
            raise ValueError("nx/ny normal zarrs must use the production uint8 encoding")
        reference_shape = tuple(int(v) for v in nx_array.shape)
    if use_spacing:
        grad_array = zarr.open(grad_mag_zarr_path, mode="r")[group]
        if reference_shape is None:
            reference_shape = tuple(int(v) for v in grad_array.shape)
        elif tuple(grad_array.shape) != reference_shape:
            raise ValueError(f"grad_mag zarr shape {grad_array.shape} differs from dense normal shape {reference_shape}")
    if scroll_zarr is not None:
        ...
        if reference_shape != expected_shape:
            print(f"WARNING: lasagna zarr shape {reference_shape} does not match ceil(scroll_zarr.shape / lasagna_scale) {expected_shape}")
    z_lo = max(0, int(np.floor(z_begin / lasagna_scale)))
    z_hi = min(reference_shape[0], int(np.ceil(z_end / lasagna_scale)))
    if z_hi <= z_lo:
        raise RuntimeError(f"lasagna z-ROI [{z_lo}, {z_hi}) is empty (zarr z size {reference_shape[0]})")
    ...
    normal_cache = (
        FastSparseCudaCache(
            source_paths=[os.path.join(normal_nx_zarr_path, group), os.path.join(normal_ny_zarr_path, group)],
            shape_zyx=reference_shape, origin_zyx=(z_lo, 0, 0),
            budget_bytes=cache_budget_bytes("normals", device), device=device,
            label="lasagna normals") if use_normals else None)
    grad_cache = (
        FastSparseCudaCache(
            source_paths=[os.path.join(grad_mag_zarr_path, group)],
            shape_zyx=reference_shape, origin_zyx=(z_lo, 0, 0),
            budget_bytes=cache_budget_bytes("grad_mag", device), device=device,
            label="lasagna grad_mag") if use_spacing else None)
    return {"backend": "sparse_cuda",
            "store": SparseLasagnaStore(normal_cache=normal_cache, grad_cache=grad_cache),
            "z_origin": z_lo, "lasagna_scale": lasagna_scale,
            "shape": (z_hi - z_lo, reference_shape[1], reference_shape[2])}
    ...
    *,
    ...
    if storage_backend != "sparse_cuda":
        raise ValueError(
            ...
            "use 'sparse_cuda'")
    if cache_backend not in ("original", "fast"):
        raise ValueError(f"unsupported sparse CUDA cache backend: {cache_backend!r}")
    ...
    if cache_backend == "fast":
        return _prepare_lasagna_volume_fast(
            scroll_zarr, use_normals=use_normals, use_spacing=use_spacing,
            normal_nx_zarr_path=normal_nx_zarr_path, normal_ny_zarr_path=normal_ny_zarr_path,
            grad_mag_zarr_path=grad_mag_zarr_path, normal_zarr_group=normal_zarr_group,
            z_begin=z_begin, z_end=z_end, lasagna_scale=lasagna_scale)
    ...
    *,
    ...
    if cache_backend == "fast":
        ...
        cache = FastSparseCudaCache(
            ...
            shape_zyx=tuple(int(v) for v in array.shape), origin_zyx=(z_lo, 0, 0),
            budget_bytes=cache_budget_bytes("sdt", torch.device("cuda")),
            device=torch.device("cuda"), label="surf_sdt")
    else:
        ...
        if progress is not None:
            progress.begin("loading", "Loading surface-distance volume onto GPU",
                           step=0, total_steps=0, unit="bricks")
        cache = ResidentBrickPool(
            sidecar, origin_zyx=(z_lo, 0, 0), z_roi=(z_lo, z_hi),
            device=torch.device("cuda"), label="surf_sdt", expected_channels=1,
            expected_shape_zyx=tuple(int(v) for v in array.shape),
            progress_callback=((lambda current, total, detail: progress.update(
                current, total_steps=total, detail=detail)) if progress is not None else None))
